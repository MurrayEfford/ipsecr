...
// 2023-01-07 includes alternative armaCHtimecpp
...
arma::cube armaCHtimecpp( 
    ...
    if (detectorcode == 2) {       
        Rcpp::stop ("timed captures not available for count detectors");
    }
    if (!(detectorcode < 2 || detectorcode == 8)) {    
        Rcpp::stop ("detectorcode not available");
    }
    ...
    // output
    arma::cube CH (N1,S,K, arma::fill::zeros);
    ...
    for (s=0; s<S; s++) {
        ...
        CH.elem(indices) = capture_time(ids);
