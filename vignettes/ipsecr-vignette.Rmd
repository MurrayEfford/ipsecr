...
**Fig. 1** Monotonic relationships between parameters (x-axis, log scale) and their default proxies (y-axis). 100 single-catch traps at 20-m spacing on a square grid. 50 simulations for each level, with other parameters held at their central value. Red line follows mean.
...
**Fig. 2** Schematic process of fitting in `ipsecr.fit`.
...
Changing the proxy function may have little effect on the estimates. Here we demonstrate an older builtin proxy function `proxyfn0` (not included in **ipsecr** 1.2):
...
**Fig. 3** Estimated trend in density from log-linear model (red). Actual trend shown as black line.
