...
```{r examplemulti, eval = runall}
secr.multi <- secr.fit(captdata, buffer = 100, detectfn = 'HHN', verbose = FALSE)
...
```{r sparse, eval = runall, echo = FALSE}
...
```{r fractional, eval = runall, message = FALSE}
library(FrF2)
NP <- 3
boxsize <- rep(0.2,3)
